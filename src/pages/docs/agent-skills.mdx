...
| `crust-mock-server` | Boot spec mocks, use stateful CRUD for round-trip flows (persisted with `--state`, seeded with `--seed`, envelope-aware), interpret `--validate` 422s, and run conformance audits with `--proxy`. | [Mock server](/docs/recipes/mock-server/) |
