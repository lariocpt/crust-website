...
<CodeSwitcher>
  ...
  # absolute, so no base URL is needed — and passing one is an error: the
...
- [Mock server](/docs/recipes/mock-server/) — pair with `verify-web-links http://localhost:PORT` to smoke-test rendered pages against mocked backends.
