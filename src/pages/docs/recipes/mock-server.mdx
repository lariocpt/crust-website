...
4. A body expands at most **20,000 schema nodes**. Past that a `$ref` to anything that can expand further terminates exactly
   ...
   every enum-constrained field in a truncated body wrong for nothing saved. Truncation still leaves a body that validates, and mock-server says so on stderr. Some real graphs are *mutually* recursive — presalytics.io/ooxml has
