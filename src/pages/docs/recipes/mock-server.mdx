...
- `additionalProperties: false` is **not** enforced by plain `--validate` — add `--strict` to enforce the literal-`false` form (see the governing rule above). It is opt-in because most specs never ask for it: of 399 APIs-guru specs, **29 (7.3%) declare it somewhere**, across 1,176 nodes. Where a spec does write it, a `--strict` rejection is that spec's own contract being honoured rather than crust inferring one.
