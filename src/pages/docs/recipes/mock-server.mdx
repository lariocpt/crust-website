...
4. A body expands at most **20,000 schema nodes**. Past that a `$ref` terminates exactly as a cycle
   does, so a truncated body still validates, and mock-server says so on stderr rather than leaving a
   short answer unexplained. Some real graphs are *mutually* recursive — presalytics.io/ooxml has
