...
<CodeSwitcher>
  ...
  mock-server --swagger ./openapi.yaml --port 4000 --state ./mock.sqlite --seed ./seed.json
...
```bash
mock-server --swagger ./things-openapi.json --port 4000 --state ./mock.sqlite --seed ./seed.json
```
...
```bash
test-pipes --target mock-state.pipes
