...
A required property that closes the loop gets the empty shape of the type it declares, and a
required **array** of the type being terminated becomes `[]` — which satisfies `required` and is
finite, where one element would be a terminating level carrying none of its own required properties
(`minItems` still wins where the spec asks for elements). Both the
