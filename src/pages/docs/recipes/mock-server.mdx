...
3. Where a node omits `type`, it is **inferred from the keywords present** — `format`, `pattern` and
   ...
   - `string` → a value that satisfies the schema's own constraints: a format-aware default for
     ...
     cannot manage degrades to a neutral value rather than a confidently wrong one; then clamped to `minLength` / `maxLength` — but **never in a way that unmakes the match just
   ...
   - `object` → every property generated — except an **optional** property crust cannot represent
     (one re-entering a cycle already expanded as far as it goes), which is **omitted** rather than
     emitted as an empty object missing its own required fields. Absent always validates;
     present-and-invalid does not. A *required* property in that position still appears, since
     presence is forced and the empty shape is then the least-bad answer. Plus any name in
     `required` that `properties` never
     describes — including where a schema has `required` and no `properties` at all, which is how a
     branch of an `allOf` adds only a requirement — and where the `required` list sits on an `allOf`
     **node** while the properties live in its branches. Such a placeholder never outranks a branch that
     DOES describe the property: turbinelabs declares `zone_key: {type: string}` in one branch and
     merely requires it in another, and the placeholder used to win. Real specs do this — ton-console's `Participant` requires `date_create` and defines
     no such property — and omitting it produced a body crust's own validator rejects. Nothing
     constrains a name with no schema, so `null` satisfies it. Under `additionalProperties: false`
     the schema forbids the key it demands; that is unsatisfiable, so crust leaves the
     contradiction visible instead of papering over it.
