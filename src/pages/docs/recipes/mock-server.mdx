...
empty shape of the type it declares — `{}` for an object, `[]` for an array, read from `type` or,
where a spec omits it, from the presence of `properties`/`items`. It used to terminate with `null`,
