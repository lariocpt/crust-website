...
```bash
test-pipes --target smoke.pipes                       # sibling smoke.setup.ts auto-detected
test-pipes --target smoke.pipes --setup ./seed.ts     # or explicit
```
...
crust -c "test-pipes --target 'tests/**/*.pipes' --bail --timeout 5000"
```
