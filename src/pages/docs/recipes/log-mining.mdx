...
```
logs> grep api_request | node_modules/.bin/pino-pretty --colorize --singleLine
[11:46:18.313] INFO: api_request {"request_id":"…","path":"/api/health","status":200,"duration_ms":4}
-- live --
…colorized lines keep streaming until ^C…
```

`--colorize` forces ANSI even though pino-pretty runs behind a pipe. Keep
the `grep`/`filter` **before** the pretty stage — after it, you'd be
matching against ANSI-decorated text.
