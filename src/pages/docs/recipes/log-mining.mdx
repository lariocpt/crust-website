...
<CodeSwitcher>
  ...
  lines **/*.log | grep ERROR | filter (l => l.includes('user=42'))
  ```
  </Fragment>
  <Fragment slot="ts">
  ```ts
...
- **`lines` vs `read` vs `Bun.file().text()`** — the shell stage `read <glob>` yields **whole-file** items (one per file), `lines <glob>` yields **one item per line**, and the TypeScript `read(path)` streams lines like the latter. They print identically on a terminal, so the difference only shows when a downstream `filter`/lambda sees one giant string instead of lines. `grep` masks it by splitting internally — which is why `read **/*.log | grep ERROR | filter (…)` works and swapping those last two stages silently does not.
